...
async def search_tickets(
    ...
    is "asc" or "desc". Returns Zendesk's default page (up to 100 results); pass the previous
    call's next_cursor to fetch more."""
    scoped_query = query if "type:" in query else f"type:ticket {query}"
    params: dict[str, Any] = {"query": scoped_query, "include": "users,groups,organizations"}
    if sort_by:
